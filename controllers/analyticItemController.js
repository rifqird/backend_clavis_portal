...
export const get_total_revenue=async(req,res)=>{
    let query=`SELECT 
        SUM(CASE WHEN aal.amount > 0 THEN aal.amount ELSE 0 END) AS total_revenue, 
        SUM(CASE WHEN aal.amount < 0 THEN aal.amount ELSE 0 END) AS total_costs, 
        SUM(aal.amount) AS net_margin 
    FROM account_analytic_line aal 
    WHERE aal.date BETWEEN '2025-12-01' AND '2026-01-31'`;
    const result = await pool.query(query);
    res.json(result.rows);
}
export const get_transaction_count=async(req,res)=>{
    let query=`SELECT 
        COUNT(*) AS transaction_count, 
        AVG(aal.amount) AS average_transaction_size 
    FROM account_analytic_line aal 
    WHERE aal.date BETWEEN '2025-12-01' AND '2026-01-31'`;
    const result = await pool.query(query);
    res.json(result.rows);
}
export const get_revenue=async(req,res)=>{
    let query=`SELECT 
        aal.date, 
        SUM(CASE WHEN aal.amount > 0 THEN aal.amount ELSE 0 END) AS revenue, 
        SUM(CASE WHEN aal.amount < 0 THEN aal.amount ELSE 0 END) AS costs 
    FROM account_analytic_line aal
    GROUP BY aal.date`;
    const result = await pool.query(query);
    res.json(result.rows);
}
export const get_distribution_project=async(req,res)=>{
    let query=`SELECT 
        aal.account_id AS project_name, 
...
export const get_partner_distribution=async(req,res)=>{
    let query=`SELECT 
        aal.partner_id AS partner_name, 
        SUM(aal.amount) AS total_contribution 
    FROM account_analytic_line aal 
    WHERE aal.date BETWEEN '2025-12-01' AND '2026-01-31' 
    GROUP BY aal.partner_id
    ORDER BY total_contribution DESC`;
    const result = await pool.query(query);
    res.json(result.rows);
}
export const get_analytic_items=async(req,res)=>{
    let query=`SELECT 
    aal.date, 
    aal.name AS description, 
    aal.account_id AS project, 
    aal.partner_id AS partner, 
    aal.amount, 
    aal.journal_id AS journal 
    FROM account_analytic_line aal 
    WHERE aal.date BETWEEN '2025-12-01' AND '2026-01-31' 
...
export const truncateInsertAnalyticItem=async(req,res)=>{
    const response = await fetch(
        `${BASE_URL}/api/account/get_analytic_items`
    );
        ...
        for (const r of rows) {
            ...
            await client.query(
                ...
                VALUES (
                    ...
                    $21::jsonb,$22::jsonb,$23::jsonb,$24::jsonb,$25::jsonb,$26,$27,$28,$29::jsonb
                )`,
                [
                    r.id,
                    r.name,
